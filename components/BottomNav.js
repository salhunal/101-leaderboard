  ...
  return (
    <>
      <button
        ...
        style={{ background: isAdmin ? "#166534" : "var(--surface)", border: `1px solid ${isAdmin ? "#16a34a" : "var(--border)"}` }}
      >
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center pb-4 px-4"
          ...
          <div className="w-full max-w-sm rounded-3xl p-6" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowLogin(false)} className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "var(--surface2)", color: "var(--muted)" }}>✕</button>
            </div>
            <form onSubmit={handleLogin} className="flex flex-col gap-3">
              ...
              <input type="password" placeholder="Şifre" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password"
                className="w-full px-4 py-3 rounded-xl text-sm"
                style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)" }} />
              {error && <p className="text-xs" style={{ color: "#fca5a5" }}>{error}</p>}
              <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-semibold mt-1"

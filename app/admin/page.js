      ...
      if (editId) {
        ...
        try {
          await fetch("/api/notify", {
            ...
            body: JSON.stringify({ players: gamePlayers, date }),
          });
        } catch {}
        setRanks(players.reduce((acc, p, i) => ({ ...acc, [p.name]: i + 1 }), {}));
        setDate(todayISO());
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => signOut(auth)} className="text-xs px-3 py-1.5 rounded-lg"
          style={{ background: "var(--surface2)", color: "var(--muted)" }}>Çıkış</button>
      </div>
      ...
      {tab === "game" && (
        <div className="rounded-2xl p-5" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          ...
          {seasons.length > 0 && (
            <div className="mb-4">
              <label className="text-xs block mb-2" style={{ color: "var(--muted)" }}>Sezon</label>
          ...
          <div className="mb-4">
            <label className="text-xs block mb-2" style={{ color: "var(--muted)" }}>Oynayan Oyuncular</label>
                ...
                return (
                  <button key={player.id}
                    ...
                    {player.photoURL
                      ...
                      : <div className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: "rgba(255,255,255,0.2)" }}>{player.name[0]}</div>
                    }
                    {player.name}
                  </button>
                );
            ...
            {selectedPlayers.length > 0 && (
              <>
                <label className="text-xs block mb-2" style={{ color: "var(--muted)" }}>Sıralama</label>
                    ...
                    return (
                      <div key={name} className="flex items-center gap-3">
                        {player?.photoURL
                          ...
                          : <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: "var(--surface2)" }}>{name[0]}</div>
                        }
                        <span className="flex-1 font-medium">{name}</span>
                        <select value={ranks[name] || 1} onChange={(e) => setRanks((prev) => ({ ...prev, [name]: Number(e.target.value) }))}
                          className="px-3 py-2 rounded-xl text-sm"
                          style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)" }}>
                          {selectedPlayers.map((_, i) => <option key={i+1} value={i+1}>{i+1}. sıra</option>)}
                        </select>
                      </div>
                    );
                  })}
          ...
          <div className="mb-4">
            <label className="text-xs block mb-2" style={{ color: "var(--muted)" }}>Not (opsiyonel)</label>
      ...
      {tab === "players" && (
        <div className="rounded-2xl p-5" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          <p className="text-sm mb-4 font-medium" style={{ color: "var(--muted)" }}>Oyuncular & Fotoğraflar</p>
          <div className="flex flex-col gap-3 mb-5">
            {players.map((p) => (
              <div key={p.id} className="rounded-xl p-3" style={{ background: "var(--surface2)" }}>
                <div className="flex items-center gap-3 mb-2">
                  {p.photoURL
                    ? <img src={p.photoURL} className="w-10 h-10 rounded-full object-cover flex-shrink-0" style={{ border: "2px solid var(--accent)" }} />
                    : <div className="w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold flex-shrink-0" style={{ background: "var(--surface)", border: "2px dashed var(--border)" }}>{p.name[0]}</div>
                  }
                  <p className="font-medium flex-1">{p.name}</p>
                  <button onClick={() => removePlayer(p.id)} className="text-xs px-2 py-1 rounded-lg"
                    style={{ background: "#450a0a", color: "#fca5a5" }}>Sil</button>
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}
                  />
                  <button onClick={() => handlePhotoSave(p)}
                    className="px-3 py-2 rounded-lg text-xs font-medium flex-shrink-0"
                    style={{ background: "var(--accent)", color: "#fff" }}>Kaydet</button>
                </div>
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            <input type="text" placeholder="Yeni oyuncu adı" value={newPlayerName} onChange={(e) => setNewPlayerName(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter" && newPlayerName.trim()) { addPlayer(newPlayerName.trim()); setNewPlayerName(""); showToast("Oyuncu eklendi ✓"); } }}
              className="flex-1 px-4 py-3 rounded-xl text-sm"
              style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)" }} />
            <button onClick={() => { if (newPlayerName.trim()) { addPlayer(newPlayerName.trim()); setNewPlayerName(""); showToast("Oyuncu eklendi ✓"); } }}
        ...
        <div className="rounded-2xl p-5" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          ...
          <div className="flex gap-2">
            <input type="text" placeholder="Sezon adı (ör: Ocak 2026)" value={newSeasonName} onChange={(e) => setNewSeasonName(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter" && newSeasonName.trim()) { addSeason(newSeasonName.trim()); setNewSeasonName(""); showToast("Sezon eklendi ✓"); } }}
              className="flex-1 px-4 py-3 rounded-xl text-sm"
              style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)" }} />
            <button onClick={() => { if (newSeasonName.trim()) { addSeason(newSeasonName.trim()); setNewSeasonName(""); showToast("Sezon eklendi ✓"); } }}
              className="px-5 py-3 rounded-xl font-semibold text-sm" style={{ background: "var(--accent)", color: "#fff" }}>Ekle</button>
          </div>
        </div>
      )}
      ...
      {tab === "settings" && (
        <div className="rounded-2xl p-5" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          ...
          <div className="mb-4">
            <label className="text-xs block mb-2" style={{ color: "var(--muted)" }}>Site Giriş Şifresi</label>
            <input type="text" placeholder="Boş bırakırsan şifre sorulmaz" value={sitePassword} onChange={(e) => setSitePassword(e.target.value)}
              className="w-full px-4 py-3 rounded-xl text-sm"
              style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)" }} />
            <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>Arkadaşların bu şifreyle siteye girecek</p>
          </div>
          <div className="mb-5">
            ...
            <div className="flex flex-wrap gap-3">
              {ACCENT_COLORS.map((c) => (
                <button key={c.value} onClick={() => setAccentColor(c.value)}
                  ...
                  {accentColor === c.value && <span className="text-white text-sm font-bold">✓</span>}
                </button>
              ))}
            </div>
          </div>
          <button onClick={handleSaveSettings} className="w-full py-3 rounded-xl font-semibold"
            style={{ background: "var(--accent)", color: "#fff" }}>Kaydet</button>
        </div>
      )}
